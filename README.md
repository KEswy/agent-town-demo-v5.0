...
## V5.5-C 会议快进 / 多语言 / 白痴变体 / 实时观战

这一轮把四块体验层一起落地，规则与决策链路保持不变：

- **会议快进**：白天会议轮到 NPC 发言时，面板出现“快进会议”按钮。后端
  `POST /api/day/npc-speeches-batch` 会沿原规则路径逐条生成待发言 NPC 的发言并推进
  会议，直到轮到玩家或会议结束；事件链、存档与重放契约与逐条生成一致，且同样支持
  `idempotency_key`（规则写入口由 20 个增至 21 个）。
- **多语言（中 / 英）**：新增 `game/scripts/localization.gd` autoload（`L10n`），设置
  弹窗可切换语言并持久化到 `agent_town_session.cfg`。核心 UI（设置、菜单、阶段 HUD、
  面板按钮、规则/战绩/复盘入口、弹窗控件、角色名/阶段名等）通过翻译表切换；后端
  生成的内容（NPC 发言、公开日志、知识库正文、复盘长文）仍以中文呈现，属于后续
  内容层翻译的待办。
- **白痴变体局**：设置弹窗新增“对局变体”（经典局 / 白痴局）。白痴局用 1 名白痴替换
  1 名村民：白天被放逐时翻牌免死一次并公开，翻牌后仍可发言但不能投票（警长投票与
  放逐投票均不可）；夜间被袭击仍正常出局；屠边判定把白痴计入民边。30 局固定 seed
  金丝雀胜率 50:50，经典 6-seed 夹具零回归，local 策略模式兼容。
- **实时观战**：浏览器打开 `http://127.0.0.1:8000/spectate` 即可观看进行中的对局，
  每 1.5 秒轮询一次公开快照（`/api/spectate/active-games` 与 `/api/spectate/{game_id}`）。
  观战页只展示公开证据：角色存活/声明/警长/白痴翻牌、阶段与发言锚点、公开时间线；
  隐藏身份、私密信息、随机种子与策略状态一律不返回。

## V5.5-D 狼战术库扩充 / NPC 个性 / 策略模式切换

- **狼战术库扩充**：夜间新增自刀银水（`wolf_self_kill.v1`，狼队按协作/欺骗强度与公开
  压力确定性决定是否自刀，女巫救则形成公开银水、不救则狼正常出局；玩家狼也可以选
  自己为目标）；发言新增假跳神职（`wolf_fake_god_claim.v1`，被施压或后期可跳守卫/
  女巫/猎人）；加上原有悍跳预言家、狼查杀狼、狼踩狼、捞队友、拉开距离、栽赃抗推、
  深水、带节奏等，狼发言战术共 11 个专属项。
- **NPC 个性化**：怀疑值累积按 aggressiveness/cautiousness 缩放（C罗更激进地升级
  怀疑、梅西需要更多证据才定性），公开声明的说服力接受度与投票评分也加入个性项，
  同一份公开信息在不同 NPC 那里会形成不同判断，不再趋同。
- **策略模式切换**：设置弹窗新增“NPC 策略模式”（规则 / 影子 / 本地模型），选择会
  持久化；对局信息栏显示当前模式。规则模式用纯 Python 规则评分（确定性、可复现）；
  本地模型用已训练策略模型接管候选评分、失败自动回退规则；影子模式只算分不行动，
  用于对比。

平衡口径（30 局固定 seed，20260601–20260630）：规则模式好人胜率 43.3%（改动前
46.7%），本地模型 53.3%；6-seed 回归夹具按意图更新为好人 5 : 狼 1。

## V5.5-E 成就解锁反馈 / 游戏内观战

- **成就解锁动画与音效**：每局结束后检测新解锁的成就（已完成局数、首胜、连胜、
  各身份首胜、MVP、验狼数、放逐命中数等 10 项），新成就会以横幅动画（滑入 + 缩放 +
  淡出，多个成就排队逐条展示）在屏幕顶部弹出，并播放合成的一串上行琶音；已解锁
  集合持久化在 `user://agent_town_achievements.cfg`，不会重复弹。
- **游戏内观战**：菜单新增“观战”，打开后直接列出后端进行中的对局，每 1.5 秒刷新
  公开快照（阶段/天数/胜负、12 名角色存活与公开声明/警长/白痴翻牌、公开时间线），
  全程在游戏窗口内完成，不再需要切到浏览器；浏览器版 `/spectate` 页面保留。

## V6.0-A 德州扑克馆（第一阶段）

- **玩法**：标准无限注德州。六人桌（1 名玩家 + 5 名 NPC），盲注 10/20，买入 1000；
  支持弃牌/过牌/跟注/加注（滑杆选额）/全下，翻牌/转牌/河牌，摊牌按手牌分底池，
  边池与平分按标准规则结算（弃牌者已投入的筹码留在底池归赢家）。
- **NPC AI**：直接复用小镇角色性格（激进/谨慎/欺骗/决策波动），按手牌强度、抽牌
  潜力和底池赔率决定弃牌/过牌/跟注/加注，偶尔诈唬。
- **入口**：小镇右上角新增“德州扑克馆”，走近按 E 进入牌桌；离开牌桌回到小镇。
- **技术**：后端 `backend/app/poker_engine.py`（纯逻辑、可 seed 复现）+ `/api/poker/*`
  REST（开桌/状态/行动/下一手），Godot 牌桌弹层在游戏窗口内完成全部操作。

## V6.0-B 世界氛围 / 场馆建筑 / 扑克战绩成就 / AI 增强

- **自由开局**：进游戏直接落在小镇，不再弹狼人杀窗口；每名 NPC 固定守在自己的
  场地做专属活动，头顶会不定期冒出彩蛋气泡——梅西/C罗在球场踢球（互呛台词）、
  周深在练歌台唱歌（《大鱼》《化身孤岛的鲸》《达拉崩吧》等歌词）、梅长苏在书房
  看书（《琅琊榜》台词）、塞尔达瞭望、小骑士练剑、大黄蜂修机械、喜羊羊/懒羊羊
  野餐、洛洛守钟楼、奇异博士守护至圣所、坏坏烤点心、然然整理信件；气泡只在自由
  状态出现，进入对局自动隐藏。走近任意 NPC 按 E 聊天；走近「德州扑克馆」按 E 进
  扑克牌桌，走近会议广场南边的「桌游馆」按 E 开始/继续狼人杀——狼人杀开始时 NPC
  才会走到专门的狼人杀场地围成一圈，平时绝不离开自己的场地。
  平时与任意小镇角色按 E 对话都会走 DeepSeek，以该角色的人设、说话风格和口头禅
  自然回应（闲逛模式下会裁剪掉“作为狼人时”的战术描述，避免出戏），并各自保留
  长期记忆；未配置 LLM 时安全回退规则回复。
- **场馆建筑**：12 处场馆全部绘制为放大、贴画风的多层建筑（球场草皮+白线+中圈+
  球门+角旗、至圣所金顶+符文圆窗+侧柱+浮空核心、点心屋暖墙+条纹棚+柜台蛋糕、
  心情邮局天蓝墙+邮筒+招牌、德州扑克馆金顶+筹码招牌+侧灯、钟楼钟面指针+尖顶+风标、
  练歌台木台+话筒+音箱、书房书架窗+台灯、训练场围栏+双靶桩+武器架、工坊烟囱+齿轮、
  瞭望台高台+望远镜+遮阳顶、野餐点格子毯+篮子+苹果），建筑彼此间隔充足、不叠场馆；
  NPC 漫游围绕建筑外围进行，不再站在建筑上；小镇上原来的 5 栋无关联房屋和 2 个
  集市棚已删除，只保留与角色相关的场馆和自然景观；场馆之间距离拉大，互不打扰。
- **扑克战绩与成就**：每手摊牌写入本地统计（手数、净输赢、单手最大赢、最佳牌型、
  顺子/同花/葫芦/四条/同花顺胜场、横扫全场次数、最近 20 手），「我的战绩」新增扑克
  段；新增 10 个扑克成就（第一手牌、百手老手、扑克首胜、各牌型赢家、横扫全场、
  筹码大亨），复用成就弹窗动画与音效。
- **AI 增强**：翻牌前按位置修正起手强度（按钮位可偷池、盲注防守折价、早位收紧）；
  两轮加注读牌——本街第二次加注后弱牌弃牌率上升、面对激进加注者只跟不加、强牌可
  对保守者 4-bet 反制；2000 手离线仿真无卡死、筹码守恒，玩家输光自动收桌、赢光
  判定“横扫全场”。
...
- 当前 379 条 MLP 使用同 seed `20260727–20260736`、默认温度 `1.0`、请求混合 `1.0`
  ...
  seed `20260601–20260630` 胜率持平、其余四项核心指标改善：胜场 `46.7%→46.7%`（持平）、误投
  `47.0%→45.4%`、投狼概率质量 `54.4%→56.8%`、放逐熵 `30.3%→28.8%`、跨日保持
  `69.5%→76.9%`，重放 `30/30`、零 fallback，金丝雀门槛通过。默认模式已随人工
...
狼人杀 21 个开局后规则写入口现在都接受可选 `idempotency_key`。契约版本为
...
- 存档升级为 3 个槽位：开局设置里选择“存档槽位”，存盘与继续上局都按当前槽位读写。
- 开局设置新增“新手阶段提示”：打开后每个阶段会在顶部显示一句该做什么的提示（不泄露隐藏身份）。
...
- 发言文案已与投票判定解耦：NPC 说服力评分只消费结构化发言计划、公开声明与立场卡，
  不再解析发言原文或按字数/关键词加分；今后调整措辞、开场白或模板都不会再悄悄改变
  平衡（解耦后金丝雀已验证）。
...
- 复盘新增“重播”：按步骤回放完整时间线，支持上一步/下一步/自动播放。
