...
这个仓库保存进入 V3 迭代前的完整可运行快照。V2.0 已覆盖十二人规则状态机、警长与警徽流、结构化普通白天发言、合法视角校验、NPC 智能参数、RAG、分层 UI、昼夜场景，以及坏坏和然然两名非参赛常驻居民。路线表中的项目是后续规划，不代表已经在 V2.0 中实现。
...
文档和自检命令不会自动启动 FastAPI 或 Godot；需要试玩时请按“运行”一节手动启动。
...
V3 不应先堆叠新角色或地图内容。推荐第一个小里程碑是建立“可复现模拟 → 指标采集 → 合法视角信念模型 → 发言与投票一致性”的闭环：先能量化警长票分布、放逐票熵、好人受骗率、LLM 校验/回退率和跨轮自相矛盾，再继续扩充战术。
