...
- 每局和批量报告当前使用 `agent_town_metrics.v4`；M02-A 当时为 metrics v1 / simulation v2，M03-A/B 为 simulation v3/v4，M04-A/B 为 v5/v6，M15-A/B 为 v7/v8，V3.1-L/M/N 为 v9/v10/v11，当前 V3.1-O 为 v12，并完整保留旧指标。
...
- M04-B 当时把模拟升级为 v6；M15-A/B 为 v7/v8，V3.1-L/M/N 为 v9/v10/v11，当前 V3.1-O 已升级到 `agent_town_simulation.v12` / `agent_town_simulation_batch.v12`，并继续保留 `speech_continuity_metrics.v1`。该统计不包含私有 belief 内容；M04-A 的警长票和放逐票 stance 对照继续保留。
...
V3.1-A 至 V3.1-I 已建立可复现模拟、核心指标、合法 belief/stance、受控发言、隐藏信息矩阵和脱敏 LLM 汇总；V3.1-J/K 完成投票概率 shadow 与首个受控消费者；V3.1-L/M/N 完成女巫、真假预言家和跨日放逐诊断；V3.1-O 已让公开争议票型驱动普通好人连续归票和女巫怀疑，并把固定 100-seed 好人胜场从 `5` 提升到 `38`。下一步应扩到 1000 seeds 和多组起始 seed 验证稳定区间，再决定是否继续平衡或进入 M09-B。
