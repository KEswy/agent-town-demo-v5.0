...
- 每局和批量报告当前使用 `agent_town_metrics.v3`；M02-A 当时为 metrics v1 / simulation v2，M03-A/B 为 simulation v3/v4，M04-A/B 为 v5/v6，M15-A/B 为 v7/v8，V3.1-L 为 v9，当前 V3.1-M 为 v10，并完整保留旧指标。
...
- M04-B 当时把模拟升级为 v6；M15-A/B 为 v7/v8，V3.1-L 为 v9，当前 V3.1-M 已升级到 `agent_town_simulation.v10` / `agent_town_simulation_batch.v10`，并继续保留 `speech_continuity_metrics.v1`。该统计不包含私有 belief 内容；M04-A 的警长票和放逐票 stance 对照继续保留。
...
- 狼人杀普通白天 NPC 发言和私聊可以按局启用云端 LLM；普通白天发言先由 LLM 在规则白名单和 `public_speech_continuity.v1` 内生成 `public_speech_plan.v3` 决策，再由独立表达调用写成角色台词，其他已接入场景继续只做角色化改写。未配置、超时或输出不安全时自动使用规则计划与模板。
...
- 普通非警长 `DAY_MEETING` 使用“决策层 → 表达层”两段流程：第一段只生成无台词的 `public_speech_plan.v3`，并消费 `public_speech_continuity.v1`；第二段只生成不含游戏事实的短语气前缀，Python 再把已校验计划渲染成完整正文。警上、警长发言、私聊及其他现有路径仍只做角色化改写。
...
- `agent_town_metrics.v3` 的赛后权限、schema 版本、空样本 `null`、票数/毒药/第二夜选择/假验人组合/条件胜负守恒、数值范围、按玩家身份/投票角色/天数完整聚合及跨进程精确重放。
- `fake_seer_campaign.v1` / `fake_seer_check_mix.v1` 的参选混合、全局随机隔离、合法验人组合和好人精确身份互换不变性。
...
V3.1-A 至 V3.1-I 已建立可复现模拟、核心指标、合法 belief/stance、受控发言、隐藏信息矩阵和脱敏 LLM 汇总；V3.1-J/K 完成投票概率 shadow 与首个受控消费者；V3.1-L 接入合法视角女巫策略，V3.1-M 完成悍跳参选/假验人混合和真假预言家链路指标。当前 100-seed 好人胜率由 3% 升至 5%，但 29 局首日成功放逐狼人也仅有 2 局好人获胜；下一步应优先审计公开证据如何跨日进入站边和后续放逐，而不是继续靠单点技能或悍跳概率补偿。
