...
- **自由开局**：进游戏直接落在小镇，不再弹狼人杀窗口；NPC 在场馆间走动，走近任意
  NPC 按 E 聊天，走近「德州扑克馆」按 E 进扑克牌桌，走近「桌游馆」按 E 开始/继续
  狼人杀。
- **场馆建筑**：12 处场馆全部绘制为贴合画风的建筑，而不是圆形色块——球场（草皮、
  白线、中圈、球门）、至圣所（金顶、符文圆窗、侧柱）、点心屋（暖墙 + 条纹棚）、
  心情邮局（天蓝墙 + 邮箱）、德州扑克馆（金顶 + 筹码招牌）、钟楼（钟面 + 尖顶）、
  练歌台（木台 + 话筒）、书房（书架窗）、训练场（围栏 + 靶桩）、工坊（烟囱冒烟）、
  瞭望台（高台 + 栏杆）、野餐点（格子毯 + 篮子）；NPC 按性格在主场与兴趣点间漫游
  （梅西/C罗常在球场、奇异博士守至圣所、懒羊羊慢悠悠）；狼人杀对局进行中 NPC
  锁定会议环位，扑克开桌时 5 名对局 NPC 移到扑克馆门口并标注“牌局中”。
