...
- 每局和批量报告当前使用 `agent_town_metrics.v4`；M02-A 当时为 metrics v1 / simulation v2，M03-A/B 为 simulation v3/v4，M04-A/B 为 v5/v6，M15-A/B 为 v7/v8，V3.1-L/M 为 v9/v10，当前 V3.1-N 为 v11，并完整保留旧指标。
...
- M04-B 当时把模拟升级为 v6；M15-A/B 为 v7/v8，V3.1-L/M 为 v9/v10，当前 V3.1-N 已升级到 `agent_town_simulation.v11` / `agent_town_simulation_batch.v11`，并继续保留 `speech_continuity_metrics.v1`。该统计不包含私有 belief 内容；M04-A 的警长票和放逐票 stance 对照继续保留。
...
V3.1-A 至 V3.1-I 已建立可复现模拟、核心指标、合法 belief/stance、受控发言、隐藏信息矩阵和脱敏 LLM 汇总；V3.1-J/K 完成投票概率 shadow 与首个受控消费者；V3.1-L/M 完成女巫和真假预言家策略诊断；V3.1-N 进一步量化跨日放逐链。当前 100-seed 好人胜率仍为 5%，首放狼人后的下一次放逐仅 `2/29` 继续命中狼人，好人 NPC 正确票跨轮保持率仅 `18.18%`。下一步可在不读取出局真值的前提下，审计并设计公开票型、既有 stance 与合法新证据的跨日消费契约。
