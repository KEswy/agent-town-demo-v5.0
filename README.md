...
- 每局和批量报告使用 `agent_town_metrics.v1`；M02-A 当时升级到 simulation v2，当前 M03-A 已继续升级为 v3 并完整保留这些指标。
...
- 狼人杀普通白天 NPC 发言和私聊可以按局启用云端 LLM；普通白天发言先由 LLM 在规则白名单内生成 `public_speech_plan.v2` 决策，再由独立表达调用写成角色台词，其他已接入场景继续只做角色化改写。未配置、超时或输出不安全时自动使用规则计划与模板。
...
- 开局设置使用独立模态窗口，可填写玩家名、选择测试身份和决定本局是否启用 LLM；也可以先关闭窗口探索小镇。
...
- 右侧“当前行动”面板只保留本阶段需要查看和提交的操作，宽度为 `440px`，展开高度约为窗口的 `62%`，并限制在 `320–520px` 之间。
- 点击“当前行动”标题栏右侧箭头可以展开或收起；内容过长时支持鼠标滚轮和触控板上下滚动，标题栏保持固定。
- “情报”使用独立的 `600px` 右侧抽屉，分为“场上角色”“公开记录”“我的记录”三页；12 张角色卡改为三列布局。
...
- 普通非警长 `DAY_MEETING` 使用“决策层 → 表达层”两段流程：第一段只生成无台词的 `public_speech_plan.v2`；第二段只生成不含游戏事实的短语气前缀，Python 再把已校验计划渲染成完整正文。警上、警长发言、私聊及其他现有路径仍只做角色化改写。
- `public_speech_plan.v2` 会明确主/次目标、支持/反对/未定立场、置信度、动作信号解读、具体追问、后续验证条件、暂定票型、战术，以及最多三项声明包、公开证据和动作信号 ID。当天通过校验的计划会随发言保存，后续投票会把暂定票作为一个有权重的依据，但新公开证据仍可推翻它。
- 决策层契约现在明确要求这些字段全部直接位于一个扁平 JSON 对象的根级，并在提示中提供完整示例，不再使用容易被模型误读为返回格式的 `fields` 描述包装。为兼容已经学到旧格式的模型，后端只会安全展开键恰好为 `schema_version + fields` 的旧 v2 包装；任何额外顶层或内层字段仍由严格 schema 拒绝。
...
- 普通发言必须选择具体目标，并至少引用一项公开动作、公开 RAG 证据或合法声明；短句“没信息，过”会被拒绝并要求改成明确判断、追问或后续验证点。LLM 可以判断错、被欺骗或错误解读公开动作的动机，但不能改写动作、身份、查验、技能、出局和胜负事实。
- “低信息量发言”是规则引擎根据公开文本生成的保守评价，不等于狼人身份，也不会自行增加怀疑值；实际状态变化仍只消费已校验计划中的结构化意图与目标。
- 普通白天的结构化意图直接驱动 NPC 状态更新；表达层不再依赖“怀疑 / 回应 / 不足以定性”等固定关键词判定意图，因此自然同义表达不会触发无意义重试。反问、否定、假设和转述中的阵营用词也不会被误认为直接身份声明。
- 公开桌面上的“4号就是狼”“我是好人”属于可能正确、也可能错误或撒谎的阵营观点，不会拿角色表里的真实身份判定是否合法。“如果他是狼，那3号可能是狼队友”、双狼猜测、引用、反问、否定，以及普通“我的队友”也允许自然表达；只有未经规则授权的第一人称明确狼队自曝，例如“我是狼”“3号是我的狼队友”“我们狼队”，继续硬拒绝。
- 结构化目标和已选动作信号是表达必须保留的下限，不再是排他的措辞上限：LLM 可以用其他公开座位作对照，也可以准确复述规则状态中已经存在的公开动作与公开声明；不存在的动作、身份、查验和技能事实仍会被拒绝。
...
- 真正篡改验人结果、改变目标、凭空增加查验或技能行动、断言未公开神职、未经授权自曝本人或明确狼队名单时，后端最多纠正五轮；全部失败后自动使用规则文本并保留审计记录，游戏流程不会卡住。
...
- 跳预言家的角色可以发布版本化警徽流；每个版本记录 `effective_night_day`、顺验目标和金水/查杀分支。场上形势变化时允许附公开理由换流，“换流”本身不扣可信度；改流理由、前后叙事和后续行动是 NPC 可以评估的公开信号。真预言家的实际夜间查验只软参考当夜有效警徽流，不被它强制锁定。
- 移交或撕毁警徽只在匹配某个明确金水/查杀分支时，才形成“按该角色自己的警徽流所表达的身份声明”。这不等于规则引擎确认验人真假；对没有写进明确另一分支的普通“没接徽”不做阵营推断。
- 玩家可在同次发言中跳预言家并发布首版警徽流，后续也可通过行动 UI 说明理由并调整；服务端在写入公开声明前对整组输入一次性校验。警徽流目标、分支和生效夜统一使用 Python 根据结构化输入生成的标准文本，玩家或 LLM 的冲突自由文本不会成为第二份事实；“目标已出局 / 出现公开身份”这类事实型换流理由也必须命中公开记录。警长当选、移徽或撕徽后，参赛角色头顶的警长标识会跟随规则状态更新。
...
- LLM 五轮校验全部失败时，对话框提供“LLM校验失败查看”；进行中的对局只显示失败原因并隐藏原始返回，服务端 JSONL 与赛后复盘保留完整原文用于审计。
...
- `belief_state.v1` 的公开/行动者私有/狼队权限、证据 ID 引用、席位覆盖、分数范围、隐藏身份与未公布夜间结果差分、声明内部来源不变性、合法狼队变化和 shadow on/off 玩法摘要一致。
- JSON 配置格式和知识条目数量。
- Python 后端编译。
...
- `public_speech_plan.v2` 的扁平根级契约、旧 `fields` 包装的安全兼容、额外字段拒绝、主次目标、立场、追问、验证条件、暂定票和战术权限，以及 v1 兼容升级、发言计划到投票的同日闭环和跨日隔离。
- `public_position.v1` 字段/精炼引用、警徽流版本与 `effective_night_day`、合理换流零默认惩罚、查验软参考、分支移徽/撕徽声明与普通未接徽不推断的边界。
- 警长票/放逐票的多种子个体公开信念、概率抽样可复现性、候选顺序不变性、强证据可共识与隐藏身份互换不变性，以及狼队软策略与“狼查杀狼”硬叙事边界。
- 玩家同次跳预言家+发布警徽流的原子提交、后续换流控件、公开摘要展示和参赛角色头顶警长标识的状态同步。
...
- `GET /api/game/{game_id}/state`：读取阶段、角色、会议、公开日志和不含真假标记的 `public_intel` 关键公开信息。
...
- 第一段 LLM 调用只返回扁平的 `public_speech_plan.v2` 策略 JSON，严格禁止 `text` 和 `fields` 包装。必填字段为 `schema_version`、`intent`、`primary_target_id`、`secondary_target_id`、`stance`、`stance_target_id`、`confidence`、`signal_read`、`question`、`verification`、`provisional_vote_target_id`、`tactic`、`claim_option_ids`、`evidence_ids`、`signal_ids`；所有字段直接位于根级，可空字段也必须显式返回 `null`。后端仅兼容键恰好为 `schema_version + fields` 的旧 v2 包装，兼容过程不会放过额外字段。
...
- 两段分别最多校验五轮。策略失败时回到原有规则计划；表达失败时保留已校验策略，并使用该策略对应的规则话术。
...
- Godot 对话框会显示“LLM（deepseek）”；每个被拒绝的候选原文都会写入 `backend/data/llm_validation_failures.jsonl`。五轮全部失败时，进行中的对局只返回脱敏占位和原因，赛后复盘可以查看完整审计原文。
...
V3.1-A 已建立可复现模拟，V3.1-B/M02-A 已建立第一版核心指标，V3.1-C/M03-A 已用影子模式锁住合法证据和变化链。下一步应先补齐 M03 的衰减/结构化私聊边界，再让 M04 的发言与投票连续性安全读取信念摘要；M06、M09 和 M15 继续后置，不应直接针对 100 个 seed 调参。
...
- 空泛过麦仍会被拒绝，但完整保留身份/查验声明的短发言不会仅因结尾出现“先看”；低信息量信号本身不能替“没信息，过”免责。
...
- 非法结构化输出最多纠正五轮，随后使用原有规则方案；进行中审计隐藏原文，服务端日志和赛后复盘保留原文。警上、警长、夜间、投票和私聊策略未在本里程碑扩展。
- README、后端说明与自动化 smoke 自检同步覆盖 JSON 适配、两段上下文隔离、非法选项、私密信息、公开人物引用、结构化状态更新和规则回退。
- 表达校验移除固定意图词表硬门槛，并区分直接阵营断言与反问、否定、假设、转述；自然的 `pressure` / `defend` 同义表达不再耗尽五轮重试，真实自曝和隐藏狼队信息仍会被拒绝。
...
- DeepSeek 内容校验扩展为最多五轮，保存原始回答与失败原因；当时调试构建曾允许游戏内查看完整记录，2026-07-16 的结构化决策更新已改为进行中脱敏、赛后查看原文。
