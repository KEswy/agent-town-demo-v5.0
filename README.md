...
- 当前 379 条 MLP 使用同 seed `20260727–20260736`、温度 `0.65`、请求混合 `0.10`
  ...
  38 次改变 teacher 首选，护栏后为 0。消融确认 `blend=0.10` 与 `blend=0.00` 的
  local 轨迹完全一致，即放逐 MLP 本身尚未改变任何采样票，此前金丝雀的指标变化
  全部来自 V5.4-A 夜间信念消费。加入置信度门禁后，10 局金丝雀为好人胜场 `2→3`、
  误投 `62.0%→58.3%`、投狼概率质量 `38.3%→43.8%`、跨日正确票保持 `80.6%→87.5%`、
  放逐熵 `25.8%→26.3%`（唯一未变好的单项，由单局轨迹翻转主导）；扩展 30 局
  seed `20260601–20260630` 上六项核心指标全部改善：胜场 `43.3%→60.0%`、误投
  `47.7%→40.6%`、投狼概率质量 `53.5%→60.5%`、放逐熵 `29.1%→28.5%`、跨日保持
  `67.0%→83.3%`，重放 `30/30`、零 fallback，金丝雀门槛通过。默认模式仍为
  `rule`；人工标签清洗（tonystark 确认 99 条）与模型重训已于 2026-08-04 完成，
  金丝雀复验通过，正式切换 `local` 为独立发布决策。
