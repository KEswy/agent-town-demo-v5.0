...
读取/启动恢复不改文件，下一次正常持久化才写入新字段。V4 公开声明还早于
`phase / window_day / event_sequence` 来源字段；只为旧 V4 快照接受缺省后得到
`"" / null / 0` 的精确默认值，并从规则摘要排除这组惰性默认。缺少非空台账、已有
带 key 事件、未知字段、非默认差异或任意摘要不一致仍 fail closed。
