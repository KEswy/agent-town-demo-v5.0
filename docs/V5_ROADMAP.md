...
| V6.0-B | P2 | 已完成 | 世界氛围与扑克深化：12 处场馆全部绘制为贴合画风的建筑（球场草皮+白线+球门、至圣所金顶+符文圆窗、点心屋条纹棚、邮局+邮箱、扑克馆金顶+筹码牌、钟楼钟面、舞台话筒、书房书架窗、训练场木桩靶、工坊烟囱、瞭望台、野餐毯）；NPC 漫游 AI（idle→walk_to→stay 状态机，速度/停留按性格，主场 58% 概率，世界边界内）；对局互斥（狼人杀进行中 NPC 锁定会议环位，扑克开桌时 5 名对局 NPC 移到扑克馆门口并显示“牌局中”）；启动不再弹狼人杀界面，先在小镇自由漫游与聊天，走近扑克馆/桌游馆按 E 进入对应游戏；扑克战绩接入本地统计与 10 个新成就（复用弹窗动画）；AI 翻牌前位置感知与两轮加注读牌；2000 手仿真无卡死、筹码守恒、决策分布合理 |
