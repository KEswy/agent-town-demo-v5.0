...
护栏后概率、方向判定与有效混合比例。
当前 379 条 MLP 的 local 金丝雀使用默认 `temperature=1.0`、请求 `blend=1.0`（狼侧
护栏封顶有效混合约 `0.5`）。消融确认 `blend≤0.25` 时模型尚未改变采样票，放宽
狼侧护栏后开始真正参与投票。2026-08-04 加入夜间信念置信度门禁、完成人工标签
清洗与重训并做平衡微调后，30 局扩展金丝雀六项核心指标全部改善（好人胜率约
43%、误投 `47.0%→45.4%`），默认模式已切换为 `local`（详见根 README）。规则
发言模板加入确定性变体后，模板重复率 `25.2%→13.1%`。
